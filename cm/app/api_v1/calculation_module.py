...
def get_integral_error(pl, interval):
    ...
    """
    return ((pl.energy_production -
             pl.prof["electricity"].sum() * interval) /
...
def run_source(kind, pl, data_in,
    ...
    if most_suitable.max() > 0:
        ...
        monthly_profile, unit, con = ru.best_unit(month_data.values,
                                                  'kWh', no_data=0,
                                                  fstat=np.median,
                                                  powershift=0)
...
def calculation(output_directory, inputs_raster_selection,
    ...
    pv_in = {'roof_use_factor':
             float(inputs_parameter_selection["roof_use_factor_pv"]/100.),
    ...
    st_in = {'roof_use_factor':
             float(inputs_parameter_selection["roof_use_factor_st"]/100.),
    ...
    reduction_factor = float(inputs_parameter_selection["reduction_factor"])/100.
    ...
    if (pv_in['roof_use_factor'] + st_in['roof_use_factor']) > 1:
        ...
        warnings.warn("""Sum of roof use factors greater than 100.
                      The roof use factor of the solar thermal has been
                      reduced to {}""".format(st_in['roof_use_factor'] * 100.))

    # define a pv plant with input features
    pv_plant = pv.PV_plant(id_plant='PV',
    ...
    if pv_plant.n_plants > 0:
        ...
        if error > 0.05:
            message = ("Difference between raster value sum and {} "
                       "profile total energy is: {:5.2}%").format(pv_plant.id,
                                                                  error * 100.)
        ...
        assert (round(pv_plant.prof["electricity"].sum()) ==
                round(pv_plant.energy_production))
    ...
    st_plant = st.ST_plant(id_plant='ST',
                           area=st_in['area'],
